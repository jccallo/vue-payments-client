...
export default {
   ...
   methods: {
      ...
      updateClient() {
         ...
         let url = "http://localhost:8000/api/client/" + this.client.id
         this.axios.put(url, this.client).then((response) => {
            this.$swal.fire(
               'Good job!',
               'You updated the client successfully!',
               'success'
            )
            console.log(response.data);
         })
